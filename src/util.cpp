...
#else
    const char* pszModule = "alohacoin";
#endif
    if (pex)
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct.  If your clock is wrong AlohaCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    ThreadSafeMessageBox(strMessage+" ", string("AlohaCoin"), wxOK | wxICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "AlohaCoin:" << FormatVersion(ALOHACOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
...
{
    return MyGetSpecialFolderPath(CSIDL_STARTUP, true) / "AlohaCoin.lnk";
}
...
{
    return GetAutostartDir() / "alohacoin.desktop";
}
