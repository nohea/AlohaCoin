...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occured. AlohaCoin can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
// TODO: implement qtipcserver.cpp for Mac and Windows
    ...
    {
        if (strlen(argv[i]) >= 7 && strncasecmp(argv[i], "alohacoin:", 7) == 0)
        {
    ...
    app.setOrganizationName("AlohaCoin");
    app.setOrganizationDomain("alohacoin.org");
    if(GetBoolArg("-testnet")) // Separate UI settings for testnet
        app.setApplicationName("AlohaCoin-Qt-testnet");
    else
        app.setApplicationName("AlohaCoin-Qt");
