...
// Copyright (c) 2016 The AlohaCoin developers
...
int nCoinbaseMaturity = COINBASE_MATURITY_ALOHA;
...
const string strMessageMagic = "AlohaCoin Signed Message:\n";
...
void static SyncWithWallets(const CTransaction& tx, const CBlock* pblock = NULL, bool fUpdate = false, bool fConnect = true)
{
    if (!fConnect)
    {
        // alohacoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // alohacoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // alohacoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// alohacoin: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(int64 nFees)
{
  ...
  do{
    ...
    if(height < BLOCKS_CONSTANT_END_OF_POW)  // 26880000 blocks (the end of POW)
      {
    nSubsidy = 21 * COIN;  // initial
    int64 nStep = height / BLOCKS_CONSTANT_OF_REWARDS_W; // per 840000 blocks
...
// alohacoin: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge, int64 nFees)
{
  ...
  int64 nStep = height / BLOCKS_CONSTANT_OF_REWARDS_S; // per 210000 blocks
...
// alohacoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // alohacoin: target change every block
    // alohacoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // alohacoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // alohacoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // alohacoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // alohacoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // alohacoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
  ...
  // alohacoin: track money supply and mint amount info
...
// alohacoin: total coin age spent in transaction, in the unit of coin-days.
...
// alohacoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // alohacoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // alohacoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // alohacoin: record proof-of-stake hash value
    ...
    // alohacoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // alohacoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // alohacoin: coinbase output should be empty if proof-of-stake block
    ...
    // alohacoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // alohacoin: check that the block satisfies synchronized checkpoint
    ...
    // alohacoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // alohacoin: check proof-of-stake
    ...
    // alohacoin: verify hash target and signature of coinstake tx
    ...
    // alohacoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // alohacoin: getblocks may not obtain the ancestor block rejected
    ...
    // alohacoin: if responsible for sync-checkpoint send it
...
// alohacoin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "AlohaCoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "AlohaCoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "05-25-2016 AlohaCoin";
        CTransaction txNew;
        txNew.nTime = 1464102000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1464102000;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1730005292;
        ...
        assert(block.hashMerkleRoot == uint256("3858654b959c3045f4d266095ca63b7a623e2693e373f7e31cbe9b0115ee3160"));
    if(false && (block.GetHash() != hashGenesisBlockOfficial))
        //if(true && (block.GetHash() != hashGenesisBlockTestNet)) // for testnet
	  {
        ...
        // alohacoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // alohacoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // alohacoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // alohacoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // alohacoin: if detected invalid checkpoint enter safe mode
    ...
    // alohacoin: if detected unmet upgrade requirement enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // alohacoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // alohacoin: record my external IP reported by peer
        ...
        // alohacoin: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // alohacoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // alohacoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // alohacoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // alohacoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // alohacoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
