...
// alohacoin: optional setting to unlock wallet for block minting only;
...
// alohacoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // alohacoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // alohacoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // alohacoin: not avatar mode
                    {
...
// alohacoin: create coin stake transaction
...
// alohacoin: check 'spent' consistency between wallet and txindex
// alohacoin: fix wallet spent state according to txindex
...
// alohacoin: disable transaction (only for coinstake)
